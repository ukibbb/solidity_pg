...
abstract contract NativeTokenSender is ProgrammableTokenSender {
    ...
    function _sendTransferRequest(
        address _to,
        uint256 _amount
    ) internal returns (bytes32 requestId) {
        if (destinationContractReceiver == address(0))
            revert NoDestinationContractReceiver();
    ...
    function _swapNativeToExchangeToken(
        uint256 _nativeTokenAmount
    ) internal returns (uint256 exchangeTokenAmount) {
        ...
        exchangeTokenAmount = ISwapRouter(uniswapV3Router).exactInputSingle{
            value: _nativeTokenAmount
        }(params);
    }
    ...
    function setDestinationContractReceiver(
        address _destinationContractReceiver
    ) external onlyOwner {
